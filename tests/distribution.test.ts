...
const jt = {
	...
	t2p: (t: number, df: number): number =>
		2 * (1 - _jt.studentt.cdf(Math.abs(t), df)),
	p2t: (p: number, df: number): number => _jt.studentt.inv(1 - p / 2, df),
	f2p: (f: number, df1: number, df2: number): number =>
		1 - _jt.centralF.cdf(f, df1, df2),
	p2f: (p: number, df1: number, df2: number): number =>
		_jt.centralF.inv(1 - p, df1, df2),
...
Deno.test('Distribution Test', () => {
	for (let i = 0; i < 1000; i++) {
		...
		assertAlmostEquals(
			pl.f2p(stats[2], df1, df2),
			jt.f2p(stats[2], df1, df2),
			1e-6,
		)

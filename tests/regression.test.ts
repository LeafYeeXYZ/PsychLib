...
const m = new Array(1000).fill(0).map(() => Math.random() * 100)
const y = new Array(1000).fill(0).map((_, i) => 0.1 * x[i] + 0.2 * m[i] + Math.random() * 1.5)
...
Deno.test('Regression Test', () => {
  ...
  assertAlmostEquals(lr2_pl.b1t!, lr2_py[1][1], 1) // t 值较大
  assertAlmostEquals(lr2_pl.b2t!, lr2_py[1][2], 1) // t 值较大
  assertAlmostEquals(lr2_pl.b1p, lr2_py[2][1], 1e-6)
  assertAlmostEquals(lr2_pl.b2p, lr2_py[2][2], 1e-6)

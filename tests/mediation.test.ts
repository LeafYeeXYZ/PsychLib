...
Deno.test('Mediation Test', () => {
  assertEquals(pl.bootstrapSample(x, m, y).x.length, x.length)
  assertEquals(pl.bootstrapSample(x, m, y).m.length, m.length)
  assertEquals(pl.bootstrapSample(x, m, y).y.length, y.length)

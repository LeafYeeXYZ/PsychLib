...
const m1: number[][] = new Array(20).fill(0).map(() => new Array(30).fill(0).map(() => Math.random()))
const m2: number[][] = new Array(30).fill(0).map(() => new Array(20).fill(0).map(() => Math.random()))
...
Deno.test('Base Test', () => {
  ...
  // Matrix Add
  assertEquals(new Matrix(m1).add(new Matrix(m1)).to2DArray(), new pl.Matrix(m1).add(new pl.Matrix(m1)).data)
  assertEquals(new Matrix(m2).add(new Matrix(m2)).to2DArray(), new pl.Matrix(m2).add(new pl.Matrix(m2)).data)
  // Matrix Multiply
  const m1_ml_mmul_m2_ml = new Matrix(m1).mmul(new Matrix(m2)).to2DArray().flat()
  const m1_pl_mmul_m2_pl = new pl.Matrix(m1).multiply(new pl.Matrix(m2)).data.flat()

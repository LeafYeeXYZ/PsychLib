...
const mA = new pl.Matrix(
	new Array(MATRIX_SIZE[0])
		.fill(0)
		.map(() =>
			new Array(MATRIX_SIZE[1]).fill(0).map(() => pl.randomNormal(MEAN, STD)),
		),
)
const mB = new pl.Matrix(
	new Array(MATRIX_SIZE[1])
		.fill(0)
		.map(() =>
			new Array(MATRIX_SIZE[0]).fill(0).map(() => pl.randomNormal(MEAN, STD)),
		),
)
const mC = new pl.Matrix(
	new Array(MATRIX_SIZE[0])
		.fill(0)
		.map(() =>
			new Array(MATRIX_SIZE[0]).fill(0).map(() => pl.randomNormal(MEAN, STD)),
...
Deno.bench(`median() - n=${N}`, () => {
	pl.median(a)
})
Deno.bench(`mode() - n=${N}`, () => {
	pl.mode(a)
})
Deno.bench(`quantile() - n=${N}`, () => {
	pl.quantile(a, 0.25)
})
Deno.bench(`range() - n=${N}`, () => {
	pl.range(a)
})
Deno.bench(`vari() - n=${N}`, () => {
	pl.vari(a)
...
Deno.bench(`kurtosis() - n=${N}`, () => {
	pl.kurtosis(a)
})
Deno.bench(`skewness() - n=${N}`, () => {
	pl.skewness(a)
})
Deno.bench(`ss() - n=${N}`, () => {
	pl.ss(a)
})
Deno.bench(`ssDiff() - n=${N}`, () => {
	pl.ssDiff(a, b)
})
Deno.bench(`sem() - n=${N}`, () => {
	pl.sem(a)
...
// Sort
Deno.bench(`Array.prototype.sort() - n=${N * 10}`, () => {
	s.slice().sort()
})
Deno.bench(`sort() - iterativeQuickSort - n=${N * 10}`, () => {
	pl.sort(s, true, 'iterativeQuickSort', false)
})
Deno.bench(`sort() - recursiveQuickSort - n=${N * 10}`, () => {
	pl.sort(s, true, 'recursiveQuickSort', false)
})
Deno.bench(`sort() - mergeSort - n=${N * 10}`, () => {
	pl.sort(s, true, 'mergeSort', false)
})
Deno.bench(`sort() - heapSort - n=${N * 10}`, () => {
	pl.sort(s, true, 'heapSort', false)
})
// Distributions
Deno.bench('z2p()', () => {
...
Deno.bench('t2p() - df=30', () => {
	pl.t2p((Math.random() - 0.5) * 10, 30)
})
Deno.bench('p2t() - df=30', () => {
	pl.p2t(Math.random(), 30)
})
Deno.bench('f2p() - df=5,30', () => {
	pl.f2p(Math.random() * 10, 5, 30)
})
Deno.bench('p2f() - df=5,30', () => {
...
Deno.bench('randomT() - df=30', () => {
	pl.randomT(30)
})
Deno.bench('randomF() - df=5,30', () => {
	pl.randomF(5, 30)
})
Deno.bench('randomChi2() - df=6', () => {
	pl.randomChi2(6)
})
// Tests
Deno.bench(`KurtosisTest - n=${N}`, () => {
	new pl.KurtosisTest(a)
})
Deno.bench(`SkewnessTest - n=${N}`, () => {
	new pl.SkewnessTest(a)
})
...
// Matrix
Deno.bench(
	`Matrix.prototype.add() - ${MATRIX_SIZE[0]}x${MATRIX_SIZE[1]}`,
	() => {
		mA.add(mA)
	},
)
Deno.bench(
	`Matrix.prototype.transpose() - ${MATRIX_SIZE[0]}x${MATRIX_SIZE[1]}`,
	() => {
		mA.transpose()
	},
)
Deno.bench(
	`Matrix.prototype.multiply() - ${MATRIX_SIZE[0]}x${MATRIX_SIZE[1]} * ${MATRIX_SIZE[1]}x${MATRIX_SIZE[0]}`,
	() => {
		mA.multiply(mB)
	},
)
Deno.bench(
	`Matrix.prototype.inverse() - ${MATRIX_SIZE[0]}x${MATRIX_SIZE[0]}`,
	() => {
		mC.inverse()
	},
)

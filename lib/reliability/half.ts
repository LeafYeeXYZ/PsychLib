...
export class HalfRealiability {
	...
	constructor(
		...
		if (Array.isArray(firstHalf[0])) {
			...
			const meanOfItems = new Array(n)
				.fill(0)
				.map((_, i) => mean(firstHalf.map((item) => (item as number[])[i])))
			first = meanOfItems
		} else {
			first = firstHalf as number[]
		}
		if (Array.isArray(lastHalf[0])) {
			...
			const meanOfItems = new Array(n)
				.fill(0)
				.map((_, i) => mean(lastHalf.map((item) => (item as number[])[i])))
			last = meanOfItems
		} else {
			last = lastHalf as number[]
		}
		if (
			first.length !== last.length ||
			(group && first.length !== group.length)
		) {
		...
		if (group) {
			...
			for (const g of this.group) {
				const result = corr(
					first.filter((_, i) => group[i] === g),
					last.filter((_, i) => group[i] === g),
				)
